...
public class TransactionServlet extends HttpServlet {
  ...
  public void InternalTransfer(HttpServletRequest request, HttpServletResponse response, Account compteTo) throws IOException {
    ...
    if (compteFrom.getBalance().compareTo(Amount) == -1) {
    ...
    } else if (bundle.getString("Echange-Devise").equals("false") && !compteFrom.getCurrency().equals(compteTo.getCurrency())) {
      ...
      ConvertedAmount = TransactionUtil.convert(compteFrom.getCurrency(), compteTo.getCurrency(), Amount);
      ...
      try {
        ...
        if (bundle.getString("Notification").equals("true")) {
          //send notification email
          Email SenderEmail = new Email();
          SenderEmail.setRecipient(transaction.getClientFrom().getEmail());
          SenderEmail.setSubject(transaction.getType());
          String content
                  = "Your transaction was Successful \n"
                  + transaction.getAmount().setScale(2, RoundingMode.FLOOR).toString() + " " + transaction.getAccountFrom().getCurrency().getSymbol()
                  + " was sent to " + transaction.getClientTo().getFullName() + ".";
          SenderEmail.setContent(content);
//////////////////////////////////
          Email ResiverEmail = new Email();
          ResiverEmail.setRecipient(transaction.getClientTo().getEmail());
          ResiverEmail.setSubject(transaction.getType());
          content = "Your have received \n"
                  + transaction.getConvertedAmount().setScale(2, RoundingMode.FLOOR).toString() + " " + transaction.getAccountTo().getCurrency().getSymbol()
                  + " from " + transaction.getClientFrom().getFullName() + ".";
          ResiverEmail.setContent(content);

          try {
            EmailUtility.sendEmail(SenderEmail);
            EmailUtility.sendEmail(ResiverEmail);
  ...
  public void B2BTransfer(HttpServletRequest request, HttpServletResponse response) throws IOException {
    ...
    if (compteFrom.getBalance().compareTo(Amount) == -1) {
    ...
    } else if (bundle.getString("Banque-vers-Banque").equals("false")) {
      session.setAttribute("transfer_error", "Banque to banque transfer is not supported");
      setCookie(response, "transfer_error", "Banque to banque transfer is not supported", 1);
      ...
      try {
        ...
        String content
                = "Your transaction was Successful \n"
                + transaction.getAmount().setScale(2, RoundingMode.FLOOR).toString() + " " + transaction.getAccountFrom().getCurrency().getSymbol()
                + " was sent to " + transaction.getClienttToName() + ".";
        SenderEmail.setContent(content);
